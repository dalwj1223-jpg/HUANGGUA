...
export interface GameStateData {
  ...
  comboCount: number; // Consecutive monster defeat streak
  comboTimer: number; // Milliseconds remaining in current combo window
}
...
/**
 * Handles defeating a monster:
 * - Increases consecutive defeat combo streak (within 2.8s window)
 * - Plays crisp "ding-dong" chime with rising pitch
 * - Awards base score + combo streak bonus
 * - Spawns burst particles & floating combo text feedback
 */
export function defeatMonster(
  state: GameStateData,
  m: Monster,
  method: 'shoot' | 'stomp' | 'ram',
  impactX?: number,
  impactY?: number
) {
  if (!m.alive) return;
  ...
  const hitX = impactX ?? (m.x + m.width / 2);
  const hitY = impactY ?? (m.y + m.height / 2);

  // 1. Combo streak counting
  if (state.comboTimer > 0) {
    state.comboCount = (state.comboCount || 0) + 1;
  } else {
    state.comboCount = 1;
  }
  // Generous 2.8s window to chain another monster defeat
  state.comboTimer = 2800;

  // 2. Play crisp "ding-dong" chime with ascending pitch based on combo count
  sound.playComboDing(state.comboCount);

  // 3. Score calculation with combo bonus
  const basePoints = method === 'shoot' ? 300 : method === 'stomp' ? 250 : 200;
  const comboBonus = (state.comboCount - 1) * 75;
  ...
  // 4. Color themed burst particles
  const burstColor =
    m.type === 'jellyfish' ? '#E879F9' : m.type === 'crab' ? '#EF4444' : '#FBBF24';
  ...
  // 5. Floating feedback score text
  state.particles.push({
    x: hitX,
    y: hitY - 12,
  ...
  // If combo streak >= 2, show prominent floating combo badge with ding!
  if (state.comboCount >= 2) {
    const comboColor =
      state.comboCount >= 6 ? '#FB7185' : state.comboCount >= 4 ? '#FB923C' : '#38BDF8';
    state.particles.push({
      x: hitX,
      y: hitY - 28,
      vx: 0,
      ...
      maxLife: 44,
      shape: 'text',
      text: `${state.comboCount}x COMBO! 叮!`,
    });
  }
}
...
export function updateGame(
  ...
  // 3.5 Combo Streak Timer (resets consecutive streak if no monster defeated in 2.8s)
  if (state.comboTimer > 0) {
    state.comboTimer -= 16.6 * deltaRatio;
    if (state.comboTimer <= 0) {
      state.comboTimer = 0;
      state.comboCount = 0;
    }
  ...
  for (const m of state.monsters) {
    ...
    if (
      ...
      defeatMonster(state, m, 'stomp', m.x + m.width / 2, m.y + m.height / 2);
    ...
    if (distToCenter < (player.width + m.width) * 0.38) {
      ...
      if (player.powerUp) {
        defeatMonster(state, m, 'ram', m.x + m.width / 2, m.y + m.height / 2);
  ...
  for (let i = state.projectiles.length - 1; i >= 0; i--) {
    ...
    for (const m of state.monsters) {
      ...
      if (
        ...
        hitMonster = true;
        defeatMonster(state, m, 'shoot', proj.x, proj.y);
        break;
      }
    }
